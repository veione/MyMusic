...
(function($){
	$.lrc = {
		...
		prefixid: 'lrc', /* 容器ID */
		hoverClass: 'lrc-hover', /* 选中节点的className */
		...
		loadLrc: function(url){
			...
			timer=setTimeout(function(){
				var lrc=$.trim($('#lrc_content').val());
					if(lrc.length>0){
						//初始化歌词信息(传入歌词文件文本)
						$.lrc.init(lrc);
					}else{
						$('#lrc_list').show().html('请收下我的膝盖,未能为你找到该歌曲歌词').addClass('load-lrc-error');
						layer.msg('请收下我的膝盖,未能为你找到该歌曲歌词');
					}
			}, 1000);
		},
		loadLocalLrc: function(){
			var lrc=$.trim($('#lrc_content').val());
			if(lrc.length>0){
				//初始化歌词信息(传入歌词文件文本)
				$.lrc.init(lrc);
				...
				$('#lrc_list').show().html('请收下我的膝盖,未能为你找到该歌曲歌词').addClass('load-lrc-error');
				layer.msg('请收下我的膝盖,未能为你找到该歌曲歌词',function(){});
			}
		},
		clear: function(){
			//播放结束后进行清除工作
			//$('#lrc_content').html('');
			$('#lrc_list').show().html('听见好音乐').css('margin-top','100px').addClass('load-lrc-finish');
		}
	};
})(jQuery);